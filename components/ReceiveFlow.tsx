...
import { X, Copy, Share2, Check, AtSign, ScanLine, Bitcoin, Landmark, ArrowRight, Loader2, Link as LinkIcon } from 'lucide-react';
...
type View = 'menu' | 'payme-id' | 'crypto-coming-soon' | 'bank-coming-soon';
...
const ReceiveFlow: React.FC<ReceiveFlowProps> = ({ onClose, selectedCurrency }) => {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(payMeLink);
    setCopied(true);
  ...
  const renderMenu = () => (
    <div className="flex flex-col gap-4 animate-fade-in pt-4">
        ...
        <GlassCard onClick={() => setView('crypto-coming-soon')} className="flex items-center gap-4 p-4 cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 active:scale-[0.98] bg-white dark:bg-white/[0.03]">
            ...
            <div className="flex flex-col">
                ...
                <span className="text-gray-500 dark:text-white/50 text-sm">Receive on-chain assets</span>
            </div>
            <div className="flex-1" />
            <ArrowRight size={20} className="text-gray-400 dark:text-white/20" />
        </GlassCard>
  ...
  return (
    <div className="absolute inset-0 z-[60] bg-[#F2F2F7] dark:bg-[#0f0b1e] flex flex-col animate-slide-up">
        ...
        <div className="flex-1 px-6 flex flex-col overflow-y-auto">
            ...
            {view === 'crypto-coming-soon' && renderComingSoon('Testnet')}
